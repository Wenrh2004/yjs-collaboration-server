...
impl ApplicationBootstrap {
    ...
    fn load_config() -> AppConfig {
        // First check whether the configuration file path is specified through the environment variable
        ...
        if AppConfig::config_exists(&config_path) {
            match AppConfig::from_yaml(&config_path) {
                Ok(config) => {
                    info!("The configuration is loaded successfully from the configuration file {}", config_path);
                    return config;
                }
                Err(e) => {
                    warn!("Failed to load from configuration file: {}, will be configured using environment variables", e);
                }
            }
        } else {
            info!("Configuration file {} does not exist, will use environment variable configuration", config_path);
        }
